...
public class Window {
     ...
     */
    public Window(String title, int height, int width, int background) {
        if (instance!=null)
            Console.close();
        this.height = height;
        this.width = width;
     ...
     * @param width in columns
     */
    public Window(String title, int height, int width) { this(title,height,width,BLACK); }

    public View getContent() {
        if (content==null)
            content = new ParentView(0,0,height,width,bkColor);
        return content;
    }
    ...
    /**
     * Put text in the message bar.
     * @param txt to write in message bar. null to clear message bar.
     */
    public void state(String txt) { say(txt); }
    ...
    public static void main(String[] args) {
        ...
        ParentView panel = new ParentView(0,0,7,12,RED);
        panel.center(20,30);
        panel.repaint();
        panel.addView(ctr);
        count(ctr,+2);
